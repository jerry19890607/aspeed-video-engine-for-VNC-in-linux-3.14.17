...
static void aspeed_video_calc_compressed_size(struct aspeed_video *video)
{
	...
	dev_dbg(video->dev, "Max compressed size: 0x%x\n",
...
static int aspeed_video_setup_video(struct aspeed_video *video)
{
	...
	strlcpy(vdev->name, DEVICE_NAME, sizeof(vdev->name));
...
static int aspeed_video_init(struct aspeed_video *video)
{
	...
	rc = devm_request_irq(dev, VIDEO_ENGINE_IRQ, aspeed_video_irq, IRQF_SHARED,
			      DEVICE_NAME, video);
	if (rc < 0) {
